...
type AuthView = 'login' | 'register' | 'reset' | '2fa';
...
export default function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (view === '2fa') {
        ...
        const res = await fetch(`${API_URL}/api/v1/auth/2fa/verify-login`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
          body: JSON.stringify({ two_factor_token: twoFactorToken, code: twoFactorCode }),
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.message || 'Kode 2FA tidak valid');
        // Successful 2FA — save token + user
        ...
        if ((response as any).two_factor_required) {
          setTwoFactorToken((response as any).two_factor_token || '');
          setTwoFactorCode('');
          setView('2fa');
          setLoading(false);
          return;
  ...
  const viewDirection = view === 'login' ? 0 : view === 'register' ? 1 : view === '2fa' ? 2 : -1;
            ...
            <div className="relative rounded-sm overflow-hidden border border-white/10 shadow-2xl bg-[#0f172a]">
              ...
              <div className="relative p-8">
                ...
                <AnimatePresence mode="wait" custom={viewDirection}>
                  ...
                  {/* ==================== 2FA CHALLENGE VIEW ==================== */}
                  {view === '2fa' && (
                    <motion.div key="2fa" custom={viewDirection} variants={slideVariants}
                      initial="enter" animate="center" exit="exit"
                      transition={{ duration: 0.3, ease: 'easeInOut' }}
                    >
                      {/* Header */}
                      <div className="text-center mb-8">
                        <div className="inline-flex p-3 rounded-sm bg-accent/5 border border-accent/10 mb-4">
                          <ShieldCheck className="w-7 h-7 text-accent" />
                        </div>
                        <h2 className="text-2xl font-black uppercase tracking-tight text-white mb-2">Two-Factor Auth</h2>
                        <p className="text-xs font-mono uppercase tracking-widest text-gray-400 max-w-xs mx-auto">
                      ...
                      <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div>
                          <label className="block text-xs font-bold font-mono text-gray-300 mb-2 uppercase tracking-widest">Kode Autentikasi</label>
                          <input
                            type="text"
                            inputMode="numeric"
                            maxLength={8}
                            ...
                            placeholder="000000"
                            className="w-full px-4 py-4 rounded-sm bg-white/[0.04] border border-white/10 text-white text-center tracking-[0.6em] text-2xl placeholder-gray-600 focus:outline-none focus:border-accent/40 transition-all"
                          />
                          <p className="text-[10px] font-mono text-gray-500 mt-2 text-center">
                            Atau masukkan recovery code (format: XXXXX-XXXXX)
                          </p>
                        </div>

                        <button type="submit" disabled={loading}
                          className="w-full py-3.5 rounded-sm bg-accent hover:bg-accent/90 text-white font-bold font-mono text-sm uppercase tracking-widest transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                          {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : (
                        ...
                        <button type="button" onClick={() => { setView('login'); setError(''); }}
                          className="w-full text-xs font-mono text-gray-500 hover:text-gray-300 transition-colors text-center">
                          ← Kembali ke Login
                        </button>
                      </form>
                    </motion.div>
