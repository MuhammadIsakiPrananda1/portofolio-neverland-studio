...
export default function HeroSection() {
  return (
    <section className="relative min-h-[500px] sm:min-h-[600px] flex flex-col items-center justify-center overflow-hidden pt-20 sm:pt-24 md:pt-32 pb-12 sm:pb-16 md:pb-20 bg-[#0B1120]">
      ...
      {/* Background glow effects — desktop only (too expensive to paint on mobile) */}
      <div className="absolute top-10 sm:top-20 right-5 sm:right-10 w-72 h-72 bg-[#ef4444]/10 rounded-full blur-[100px] hidden sm:block" />
      <div className="absolute bottom-10 sm:bottom-20 left-5 sm:left-10 w-72 h-72 bg-[#ef4444]/10 rounded-full blur-[100px] hidden sm:block" />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 md:w-[600px] h-96 md:h-[600px] bg-slate-800/20 rounded-full blur-[100px] hidden sm:block" />

      {/* Content Container */}
      <div className="container relative z-10 w-full max-w-6xl mx-auto px-4 sm:px-6 md:px-8 mt-4 sm:mt-8 md:mt-12">
        <div className="flex flex-col items-center text-center w-full">

          {/* H1 — immediately visible via CSS (critical for LCP) */}
          <div className="mb-6 flex flex-col items-center max-w-5xl hero-slide-up">
          ...
          {/* Description Text */}
          <p className="hero-fade-in text-sm sm:text-base md:text-lg text-slate-400 font-medium mb-8 sm:mb-10 max-w-3xl mx-auto leading-relaxed px-4">
            {COMPANY_INFO.description}
          </p>

          {/* CTA Buttons */}
          <div className="hero-fade-in-2 flex flex-row flex-wrap items-center justify-center gap-3 sm:gap-4 mb-12 sm:mb-16">
          ...
          <div className="hero-fade-in-2 w-full mt-4 mb-6 sm:mb-8">
          ...
          {/* Cards Grid — CSS hover, no framer-motion */}
          <div className="hero-fade-in-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 w-full p-2">
            {HERO_STATS.map((item, index) => (
              <div
                key={index}
                className="bg-[#0f172a]/60 border border-white/10 rounded-xl p-3 sm:p-4 flex items-center justify-between shadow-lg hover:shadow-xl hover:border-white/20 hover:-translate-y-1 hover:scale-[1.01] transition-[border-color,box-shadow,transform] duration-200 group"
              >
                <div className="flex items-center gap-3 sm:gap-4">
                  {/* Left Icon Block */}
                  <div className="p-2 sm:p-3 bg-white/5 border border-white/10 rounded-lg group-hover:bg-white/10 group-hover:border-white/20 transition-[background-color,border-color] duration-200">
                    <item.icon className="w-5 h-5 sm:w-6 sm:h-6 text-slate-300 group-hover:text-white transition-colors duration-200" />
                  </div>
                  {/* Card Details */}
                ...
                <div className="flex items-center justify-center p-1 sm:p-2 ml-2 border border-white/10 rounded-full bg-white/5">
                  <div className={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full ${item.status} shadow-[0_0_8px_rgba(34,197,94,0.6)]`} />
                </div>
              </div>
            ))}
          </div>
